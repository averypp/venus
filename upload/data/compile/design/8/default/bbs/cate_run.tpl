...
<div class="tac"> </div>
	<div class="main_wrap">
		...
		<div class="bread_crumb">
			<?php echo $headguide;
  if ($tab == 'digest') { ?><em>&gt;</em><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?c=cate&fid=', rawurlencode($fid),'&tab=digest'; ?>">精华</a><?php }?>
		</div>
	...
	<?php  } ?>
					...
					<div class="box_wrap thread_page J_check_wrap">
						...
						<div class="content_nav" id="hashpos_ttype">
							...
							<div class="content_filter"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?c=cate&fid=', rawurlencode($fid); 
 echo htmlspecialchars($_urladd_, ENT_QUOTES, 'UTF-8');
 echo htmlspecialchars($defaultOrderby == 'lastpost' ? '&orderby=postdate' : '', ENT_QUOTES, 'UTF-8');?>"  class="<?php echo htmlspecialchars(Pw::isCurrent($orderby == 'postdate'), ENT_QUOTES, 'UTF-8');?>">最新发帖</a><span>|</span><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?c=cate&fid=', rawurlencode($fid); 
 echo htmlspecialchars($_urladd_, ENT_QUOTES, 'UTF-8');
 echo htmlspecialchars($defaultOrderby == 'lastpost' ? '' : '&orderby=lastpost', ENT_QUOTES, 'UTF-8');?>" class="<?php echo htmlspecialchars(Pw::isCurrent($orderby != 'postdate'), ENT_QUOTES, 'UTF-8');?>">最后回复</a></div>
							<ul class="cc">
								<li class="<?php echo htmlspecialchars(Pw::isCurrent(!$tab), ENT_QUOTES, 'UTF-8');?>"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?c=cate&fid=', rawurlencode($fid); ?>">全部</a></li>
								<li class="<?php echo htmlspecialchars(Pw::isCurrent('digest' == $tab), ENT_QUOTES, 'UTF-8');?>"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?c=cate&fid=', rawurlencode($fid),'&tab=digest'; ?>">精华</a></li>
							</ul>
						</div>
						</nav>
						<?php  if ($threaddb) { ?>
								...
								<tr>
									...
									<td class="subject">
										<p class="title">
											...
											<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','read.php?tid=', rawurlencode($value['tid']),'&fid=', rawurlencode($value['fid']); ?>" class="st" style="<?php echo htmlspecialchars($value['highlight_style'], ENT_QUOTES, 'UTF-8');?>"><?php echo htmlspecialchars(Pw::substrs($value['subject'],$numofthreadtitle), ENT_QUOTES, 'UTF-8');?></a>
											<?php
PwHook::display(array($threadList, "runDo"), array('createHtmlAfterSubject',$value), "", $__viewer);
  ...
  if ($value['ifupload']) { ?><span class="posts_icon"><i class="icon_<?php echo htmlspecialchars($uploadIcon[$value['ifupload']], ENT_QUOTES, 'UTF-8');?>" title="<?php echo htmlspecialchars($icon[$uploadIcon[$value['ifupload']]], ENT_QUOTES, 'UTF-8');?>"></i></span><?php  } ?>
										...
										<p class="info">
											...
											最后回复：<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=space&uid=', rawurlencode($value['lastpost_userid']); ?>"><?php echo htmlspecialchars($value['lastpost_username'], ENT_QUOTES, 'UTF-8');?></a><span><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','read.php?tid=', rawurlencode($value['tid']),'&fid=', rawurlencode($value['fid']),'&page=e'; ?>#a" rel="nofollow"><?php  echo Pw::time2str($value['lastpost_time'], 'm-d H:i');?></a></span>
										</p>
									</td>
									<td class="num">
...
?>
	<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?c=cate&fid=', rawurlencode($fid),'&page=', rawurlencode($_page_i); 
