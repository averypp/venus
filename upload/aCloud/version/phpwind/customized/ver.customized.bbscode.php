...
function cvurl($http,$url='',$name='',$ifdownload='',$checkurl) {
	...
	if (!$url) {
		$tempUrl = parseUrl('http://www.'.$http,'www.'.$http);
		$url = $tempUrl ? $tempUrl : "<a href=\"http://www.$http\" target=\"_blank\" $addjs>www.$http</a>";
	} elseif (!$name) {
		$tempUrl = parseUrl($http.$url,$http.$url);
		$url = $tempUrl ? $tempUrl : "<a href=\"$http$url\" target=\"_blank\" $addjs>$http$url</a>";
	} elseif (!$http && $url) {
		if (strrpos($url, 'u.php?uid=') !== false) {
			$tempUrl = parseUrl($db_bbsurl.'/'.$url,$name);
			preg_match_all('/^u.php\?uid=(\d*)$/i', $url, $uids);
			$uid = $uids[1][0];
			$url = $db_bbsurl.'/'.$url;
			$url = $tempUrl ? $tempUrl : "<a class=\" _cardshow\" href=\"$url\" data-card-key=\"$name\" data-card-url=\"pw_ajax.php?action=smallcard&type=showcard&uid=".$uid."\" target=\"_blank\" $addjs>$name</a>";
		} else {
			$tempUrl = parseUrl('http://'.$url,$name);
			$url = $tempUrl ? $tempUrl : "<a href=\"http://$url\" target=\"_blank\" $addjs>$name</a>";
		}
	} elseif (!$ifdownload) {
		$tempUrl = parseUrl($url,$name);
		$url = $tempUrl ? $tempUrl : "<a href=\"$http$url\" target=\"_blank\" $addjs>$name</a>";
	} else {
...
function parseUrl($url,$name){
	global $db_bbsurl;
	preg_match('/^http(s)?:\/\/(.+\.)*((.+)\.(com|net|cn|com\.cn|net\.cn|org|org\.cn|biz|cc|name|asia|mobi|me|tel|中国|公司|网络|hk|tv))(\/(.+))?/i',$db_bbsurl,$matches);
	if (strpos($url,$matches[3]) !== false) {
		return getParseUrl($url,$name);
	}
}

function getParseUrl($url,$name) {
	global $db_dir,$db_ext;
	if (preg_match('/u\.php\?uid=(\d*)$/i', $url, $matches)) {
		return '<topic type="3" id="'.$matches[1].'">'.$name.'</topic>';
	} elseif (preg_match('/read\.php\?tid=(\d*)#?(\d*)(.*)?/i', $url, $matches) || preg_match('/read'.$db_dir.'tid-(\d*)'.$db_ext.'#?(\d*)(.*)?/i', $url, $matches)) {
		return '<topic type="1" id="'.$matches[1].','.$matches[2].'">'.$name.'</topic>';
	} elseif (preg_match('/thread\.php\?fid=(\d*)(.*)/i', $url, $matches) || preg_match('/thread'.$db_dir.'fid-(\d*)(.*)/i', $url, $matches)) {
		return '<topic type="2" id="'.$matches[1].'">'.$name.'</topic>';
	} 
}
...
function qoute($code) {
	...
	$code_num++;
	$code = preg_replace("/\[url=(.*?)\/job\.php\?action=topost(.*?)\]<img(.*?)>\[\/url\]/eis","",$code);
	$code_htm[6][$code_num]="<blockquote>".str_replace('\\"','"',$code)."</blockquote>";
	return "<\twind_code_$code_num\t>";
}
function ifpost($tid) {
...
function post($code) {
	...
	if (ifpost($tid) > 0) {
		$r_ifpost = ifpost($tid);
		$code_htm[3][$code_num] = $code;
	} else {
		$code_htm[3][$code_num] = "<blockquote>本部分内容设定了隐藏,需要回复后才能看到</blockquote>";
	}
	return "<\twind_code_$code_num\t>";
}
function hidden($cost,$code) {
	...
	if ($windid != $tpc_author && $usercredit < $creditvalue && !$isGM && !$pwEncodeHide || $groupid == 'guest') {
		$code = "<blockquote>本部分内容设定了加密,查看此帖需要{$creditname}:{$creditvalue}</blockquote>";
	}
	$code_htm[4][$code_num] = $code;
	return "<\twind_code_$code_num\t>";
}
function sell($cost,$code) {
	global $isGM,$windid,$winduid,$tpc_author,$tpc_pid,$fid,$tid,$i_table,$groupid,$code_num,$code_htm, $db_bbsurl,$db_sellset,$_CREDITDB,$pwSellHide,$sell_num,$tpc_buy;
	$code_num++;
	list($creditvalue,$credittype) = explode(',',$cost);
	...
	if (++$sell_num == 1) {
		$printcode = "<blockquote>此帖售价 ".$creditvalue." ".$creditname."  <topic type=\"sell\" id=\"".$tid.",".$tpc_pid."\">立即购买</topic></blockquote>";
	}
	if ($isShow) {
		$printcode = "<blockquote>此帖售价 ".$creditvalue." ".$creditname." 以下为出售内容<br />"
					. str_replace('\\"','"',$code)
					. "</blockquote>";
	}
...
function wplayer($wmvurl,$width='',$height='',$auto='',$type='wmv'){
	...
	!$width && $width = 314;
	!$height && $height = 256;
    return (++$player_id == 1 ? "<script id=\"js_player\" src=\"js/player.js\"></script>" : '')."<div id=\"player_$player_id\"><a href=\"$wmvurl\" class=\"video\" data-pid=\"$player_id\" data-url=\"$wmvurl\" data-width=\"$width\" data-height=\"$height\" data-type=\"$type\">视频播放</a></div>".($auto == '1' ? "<script type=\"text/javascript\">player('player_{$player_id}','$wmvurl','$width','$height','$type');</script>" : '');
}
