	...
	<div class="main_wrap">
		...
		<div class="forum_page">
			<div class="bbs_info_box">
				<span>今日：<?php echo htmlspecialchars($todayposts, ENT_QUOTES, 'UTF-8');?></span><i></i><span>昨日：<?php echo htmlspecialchars($bbsinfo['yposts'], ENT_QUOTES, 'UTF-8');?></span><i></i><span>最高日：<?php echo htmlspecialchars($bbsinfo['hposts'], ENT_QUOTES, 'UTF-8');?></span><i></i><span>帖子：<?php echo htmlspecialchars($article, ENT_QUOTES, 'UTF-8');?></span><i></i><span>会员：<?php echo htmlspecialchars($bbsinfo['totalmember'], ENT_QUOTES, 'UTF-8');?></span><i></i><span>新会员：<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=space&username=', rawurlencode($bbsinfo['newmember']); ?>" target="_blank"><?php echo htmlspecialchars($bbsinfo['newmember'], ENT_QUOTES, 'UTF-8');?></a></span>
			</div>
			...
			<div class="box_wrap">
				...
				<div class="forum_group">
					<div class="box_title">
						<div class="fr">
							<?php if ($_cate['manager']) {?>
								分栏版主：
							<?php foreach ($_cate['manager'] as $_name) {?>
								<a class="J_user_card_show" data-username="<?php echo htmlspecialchars($_name, ENT_QUOTES, 'UTF-8');?>" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=space&username=', rawurlencode($_name); ?>"><?php echo htmlspecialchars($_name, ENT_QUOTES, 'UTF-8');?> </a>
							<?php }
 }?>
						</div>
						<h2 class="cname"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?c=cate&fid=', rawurlencode($_cateId); ?>"><?php echo $_cate['name'];?></a></h2>
					</div>
					<?php if ($_cate['across'] > 1) {
 PwHook::segment("forum_list_table", array($_cate,$forumList[$_cateId]), "more_across", "", $__viewer); 
 } else {
 PwHook::segment("forum_list_table", array($_cate,$forumList[$_cateId]), "one_across", "", $__viewer); 
 }?>
				</div>
				<?php  } ?>
			</div>
