...
?>
<link href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'themes').'/site/default/css'.Wekit::getGlobal('theme','debug'); ?>/task.css?v=<?php echo htmlspecialchars(NEXT_RELEASE, ENT_QUOTES, 'UTF-8');?>" rel="stylesheet" />
</head>
<body>
<div class="wrap">
...
<div class="tac"> </div>
	<div class="main_wrap">
		<div class="bread_crumb">
			<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/',''; ?>" class="home" title="<?php echo Wekit::C("site", "info.name"); ?>">首页</a><em>&gt;</em><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=task'; ?>">任务</a><em>&gt;</em><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=task'; ?>">进行中</a>
		</div>
		<div class="main cc">
			<div class="main_body">
				<div class="main_content">
					...
					<nav>
						<div class="content_nav">
							<ul>
								<li class="current"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=task'; ?>">进行中</a></li>
								<li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=task&a=applicableList'; ?>">可领取</a></li>
								<li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=task&a=completeList'; ?>">已完成</a></li>
							</ul>
						</div>
					</nav>
		<?php if ($count == 0) {?>
						<div class="not_content">啊哦，暂没有进行中的任务，马上去<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=task&a=applicableList'; ?>">领取任务</a>吧！</div>
		<?php }else{ ?>
		...
		?>
								<dl id="J_task_<?php echo htmlspecialchars($item['taskid'], ENT_QUOTES, 'UTF-8');?>">
									<dt>
							<?php if ($item['icon']) {?>
									<img src="<?php echo Wind::getComponent('response')->getData('G', 'url', 'themes').'/site/default/images'; ?>/common/blank.gif" width="80" height="80" style="background:url(<?php echo htmlspecialchars(Pw::getPath($item['icon']), ENT_QUOTES, 'UTF-8');?>) no-repeat;" alt="<?php echo htmlspecialchars($item['title'], ENT_QUOTES, 'UTF-8');?>" />
							<?php }else{?>
									<img src="<?php echo Wind::getComponent('response')->getData('G', 'url', 'themes').'/site/default/images'; ?>/common/blank.gif" width="80" height="80" style="background:url(<?php echo Wind::getComponent('response')->getData('G', 'url', 'themes').'/site/default/images'; ?>/task/none.png) no-repeat;" alt="<?php echo htmlspecialchars($item['title'], ENT_QUOTES, 'UTF-8');?>" />
	    					<?php }?>
									</dt>
									<dd>
										<p><strong class="name"><?php echo htmlspecialchars($item['title'], ENT_QUOTES, 'UTF-8');?></strong></p>
										<?php if ($item['percent']) {?><p>进度：<span class="progress_wrap"><span class="progress"><span style="width:<?php echo htmlspecialchars($item['percent'], ENT_QUOTES, 'UTF-8');?>;"></span></span><em class="num"><?php echo htmlspecialchars($item['percent'], ENT_QUOTES, 'UTF-8');?></em></span></p><?php }?>
										<p>目标：<?php echo $item['description'];?></p>
										<p>时限：<?php echo htmlspecialchars($time, ENT_QUOTES, 'UTF-8');?></p>
										<p>奖励：<span class="green"><?php echo htmlspecialchars($reward, ENT_QUOTES, 'UTF-8');?></span></p>
										
										<p class="tar">
		<?php if (!$item['continue']) {?><button class="btn disabled" disabled><?php echo htmlspecialchars($modes[$item['tag']]['button'], ENT_QUOTES, 'UTF-8');?></button>
		<?php }else{
		...
		?><a href="<?php echo htmlspecialchars(WindUrlHelper::createUrl($url), ENT_QUOTES, 'UTF-8');?>" data-id="<?php echo htmlspecialchars($item['taskid'], ENT_QUOTES, 'UTF-8');?>" class="btn <?php echo htmlspecialchars($cls, ENT_QUOTES, 'UTF-8');?>"><?php echo htmlspecialchars($modes[$item['tag']]['button'], ENT_QUOTES, 'UTF-8');?></a>
		<?php }?>
										</p>
									</dd>
								</dl>
...
?>
	<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=task&page=', rawurlencode($_page_i); 
