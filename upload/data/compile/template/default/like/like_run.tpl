...
?>
<link href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'themes').'/site/default/css'.Wekit::getGlobal('theme','debug'); ?>/like.css?v=<?php echo htmlspecialchars(NEXT_RELEASE, ENT_QUOTES, 'UTF-8');?>" rel="stylesheet" />
</head>
...
<style type="text/css">
    			...
    			$__design_data = $__design->getDataByModules(array(1,));
...
<div class="tac"> </div>
	<div class="main_wrap">
		...
		<div class="bread_crumb">
			<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/',''; ?>" class="home" title="<?php echo Wekit::C("site", "info.name"); ?>">首页</a><em>&gt;</em><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=like&c=like'; ?>">喜欢</a><em>&gt;</em><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=like&c=like'; ?>">热门喜欢</a>
		</div>
		<div class="like_page_run cc">
			<div class="like_nav cc">
				<ul>
					<li  class="current"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=like&c=like'; ?>">热门喜欢</a></li>
					<li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=like&c=mylike&a=ta'; ?>" data-referer="true" class="J_qlogin_trigger" rel="nofollow">Ta的喜欢</a></li>
					<li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=like&c=mylike'; ?>" data-referer="true" class="J_qlogin_trigger" rel="nofollow">我的喜欢</a></li>
				</ul>
			</div>
			<div class="like_falls cc">
				<?php $datamodule = "1"; $pageid = "1"; $template="<div class=\"tmode_waterfall\"><% if(thumb != \"\"){ %><div class=\"img\"><a href=\"<%=url%>\" target=\"_blank\"><img src=\"<%=thumb%>\" width=\"200\" alt=\"<%=title%>\"></a></div><% }%><div class=\"title\"><a href=\"<%=url%>\" target=\"_blank\"><%=title%></a></div><div class=\"descrip\"><%=intro%></div><div class=\"user\"><a href=\"<%=space%>\" target=\"_blank\" data-uid=\"<%=uid%>\" class=\"J_user_card_show\"><img src=\"<%=avatar_s%>\" width=\"30\" height=\"30\"><%=author%></a><span class=\"time\"><%=threadTime%></span></div><div class=\"num\"><span class=\"icon_like\" title=\"喜欢\"><%=like%></span><span class=\"icon_reply\" title=\"回复\"><%=replies%></span></div></div>"; $getdata=1;?>
...
<script>
var J_LIKE_URL = "<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=like&c=like&a=data&moduleid=', rawurlencode($datamodule),'&pageid=', rawurlencode($pageid),'&page=', rawurlencode($page); ?>",getdata="<?php echo htmlspecialchars($getdata, ENT_QUOTES, 'UTF-8');?>";
var LIKE_PLUS = "<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=like&c=mylike&a=doLike&typeid=_FROMTYPE&fromid=_KEY'; ?>";		//点击喜欢地址
var J_LIKE_DATA = "<?php echo WindSecurity::escapeEncodeJson($data, 'UTF-8');?>";
