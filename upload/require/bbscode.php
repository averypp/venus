...
//iframe没开启的情况
function setIframeNo($path){
    $path = escapeUrl($path);
    if($path) {
        return "Iframe Close: <a target=_blank href='".$path." '>".$path."</a>";;
    }else{
        return "";
    }
}
function setVideo($message){
	$exts=array('rmvb','rm','wmv','avi','mp3','wma','swf','flv');
	if(preg_match_all("/\[flash=(\d+?)\,(\d+?)(\,(0|1))?\]([^\[\<\r\n\"']+?)\[\/flash\]/iU", $message, $matches)) {
		foreach($matches[0] as $k => $v) {
			$pos=strrpos($matches[5][$k],'.');
			if ($pos!==false){
                $matches[5][$k] = escapeUrl($matches[5][$k]); //by taishici
				$ext=substr($matches[5][$k],$pos+1);
				if (S::inArray($ext,$exts)){
					$str=wplayer($matches[5][$k],$matches[1][$k],$matches[2][$k],$matches[4][$k],$ext);
					$message = str_replace($v, $str, $message);
				}else{
					$message = str_replace($v, $matches[5][$k], $message);
				}
			}
		}
	}
...
function setAudio($message){
	$exts=array('rmvb','rm','wmv','avi','mp3','wma','swf','flv');
	if(preg_match_all("/\[(wmv|mp3)=([01]{1})?\]([^\<\r\n\"']+?)\[\/\\1\]/is", $message, $matches)) {
		foreach($matches[0] as $k => $v) {
			$pos=strrpos($matches[3][$k],'.');
			if ($pos!==false){
                $matches[3][$k] = escapeUrl($matches[3][$k]); //by taishici
				$ext=substr($matches[3][$k],$pos+1);
				if (S::inArray($ext,$exts)){
					$str=wplayer($matches[3][$k],'314','53',$matches[2][$k],$ext);
					$message = str_replace($v, $str, $message);
				}else{
					$message = str_replace($v, $matches[3][$k], $message);
				}
			}
		}
	}
	if(preg_match_all("/\[(wmv|rm)(=([0-9]{1,3})\,([0-9]{1,3})\,(0|1))?\]([^\<\r\n\"']+?)\[\/\\1\]/is", $message, $matches)) {
		foreach($matches[0] as $k => $v) {
			$pos=strrpos($matches[6][$k],'.');
			if ($pos!==false){
                $matches[6][$k] = escapeUrl($matches[6][$k]); //by taishici
				$ext=substr($matches[6][$k],$pos+1);
				if (S::inArray($ext,$exts)){
					$str=wplayer($matches[6][$k],$matches[3][$k],$matches[4][$k],$matches[5][$k],$ext);
					$message = str_replace($v, $str, $message);
				}else{
					$message = str_replace($v, $matches[6][$k], $message);
				}
			}
		}
	}
...
function sell($cost,$code) {
	...
	$printcode = '';
	$buyBaseUrl = "job.php?action=buytopic&tid=$tid&pid=$tpc_pid&verify=$GLOBALS[verifyhash]&buyPage=1&";
	if (++$sell_num == 1) {
...
function wplayer($wmvurl,$width='',$height='',$auto='',$type='wmv'){
	...
	!$height && $height = 256;
	++$player_id;
    return "<div id=\"player_$player_id\">".($auto == '1' ? "<script type=\"text/javascript\">player('player_{$player_id}','$wmvurl','$width','$height','$type');</script>":"<a href=\"$wmvurl\" class=\"video\" data-pid=\"$player_id\" data-url=\"$wmvurl\" data-width=\"$width\" data-height=\"$height\" data-type=\"$type\">视频播放</a>")."</div>";
}
